CLASS lhc_Inventory DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS CalculateInventoryID FOR DETERMINE ON MODIFY
...
CLASS lhc_Inventory IMPLEMENTATION.
  ...
  METHOD CalculateInventoryID.
    ...
    "Ensure idempotence
    READ ENTITIES OF zi_rap_inventory_7139 IN LOCAL MODE
      ENTITY Inventory
        FIELDS ( InventoryID )
    ...
    "Get max travelID
    SELECT SINGLE FROM zrap_inven_7139 FIELDS MAX( inventory_id ) INTO @DATA(max_inventory).

    "update involved instances
    MODIFY ENTITIES OF zi_rap_inventory_7139 IN LOCAL MODE
      ENTITY Inventory
  ...
  METHOD GetPrice.
    ...
    DATA proxy TYPE REF TO zrap_7139_co_epm_product_soap .
    ...
    LOOP AT inventories ASSIGNING FIELD-SYMBOL(<inventory>).
      ...
      TRY.
          ...
          ENDIF.
          IF proxy IS INITIAL.
            proxy = NEW zrap_7139_co_epm_product_soap(
                             destination = destination
                           ).
          ENDIF.

          DATA(request) = VALUE zrap_7139_req_msg_type( req_msg_type-product = <inventory>-ProductID ).
          proxy->get_price(
            EXPORTING
              input = request
            IMPORTING
        ...
        CATCH zrap_7139_cx_fault_msg_type INTO DATA(soap_exception).
