CLASS zcl_rslt_rap_ce_agency DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_agency_range TYPE RANGE OF zz_rslt_rap_travel_agency-agencyid.
    TYPES t_business_data TYPE TABLE OF zz_rslt_rap_travel_agency.
...
CLASS zcl_rslt_rap_ce_agency IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    TRY.
        ...
        get_agencies(
            EXPORTING
              filter_cond        = io_request->get_filter( )->get_as_ranges( )
              top                = CONV i( io_request->get_paging( )->get_page_size( ) )
              skip               = CONV i( io_request->get_paging( )->get_offset( ) )
  ...
  METHOD get_agencies.
    ...
    DATA:
      ...
      filter_node_root TYPE REF TO /iwbep/if_cp_filter_node.

    " Create http client
    " Details depend on your connection settings
    http_client = cl_web_http_client_manager=>create_by_http_destination(
        cl_http_destination_provider=>create_by_url( i_url = 'https://sapes5.sapdevcenter.com' ) ).

    client_proxy = cl_web_odata_client_factory=>create_v2_remote_proxy(
        iv_service_definition_name = 'ZSC_RSLT_RAP_AGENCY'
        io_http_client             = http_client
        iv_relative_service_root   = '/sap/opu/odata/sap/ZAGENCYCDS_SRV/' ).

    " Navigate to the resource and create a request for the read operation
    request =
        client_proxy->create_resource_for_entity_set( 'Z_TRAVEL_AGENCY_ES5' )->create_request_for_read( ).

    " Navigate to the resource and create a request for the read operation
    DATA(read_list_request) =
        client_proxy->create_resource_for_entity_set( 'Z_TRAVEL_AGENCY_ES5' )->create_request_for_read( ).
    ...
    LOOP AT  filter_cond  INTO DATA(filter_condition).
      ...
      IF filter_node_root IS INITIAL.
        filter_node_root = filter_node.
      ELSE.
        filter_node_root = filter_node_root->and( filter_node ).
      ENDIF.
    ENDLOOP.

    IF filter_node_root IS NOT INITIAL.
      read_list_request->set_filter( filter_node_root ).
    ENDIF.
